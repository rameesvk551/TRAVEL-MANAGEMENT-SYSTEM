...
// Recharts is not installed, using simple placeholders or Ant Design Charts if avail
// For this demo, we will use simple stats and mock chart visual
...
const WhatsAppAnalytics: React.FC = () => {
    // Fetch stats
    useQuery({
        queryKey: ['whatsapp-stats-campaigns'],
        queryFn: () => whatsappApi.getCampaignStats(),
    });

    useQuery({
        queryKey: ['whatsapp-stats-response'],
        queryFn: () => whatsappApi.getResponseStats(),
    });
                ...
                <Col span={12}>
                    <Card title="Message Volume Trend" style={{ height: 400 }}>
                        <div style={{ height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f5', borderRadius: 8 }}>
                            ...
                            {/* In a real app, render Recharts App here */}
                        </div>
                    </Card>
                </Col>
                <Col span={12}>
