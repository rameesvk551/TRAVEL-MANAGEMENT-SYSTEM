...
export class LeadRepository {
    ...
    async update(tenantId: string, id: string, data: UpdateLeadDTO) {
        ...
        // Merge collected_data instead of replacing
    ...
    async updateLastMessageAt(tenantId: string, phone: string) {
        ...
        if (lead) {
            await lead.update({ last_message_at: new Date() });
            return lead;
        }
        return null;
    }
    ...
    async getStats(tenantId: string) {
        ...
        const newToday = await Lead.count({
            where: { tenant_id: tenantId, created_at: { [Op.gte]: today } },
        });

        const newThisWeek = await Lead.count({
            where: { tenant_id: tenantId, created_at: { [Op.gte]: startOfWeek } },
        });
    ...
    async bulkUpdateStatus(tenantId: string, ids: string[], status: LeadStatus) {
        ...
        const [count] = await Lead.update(updateData, {
