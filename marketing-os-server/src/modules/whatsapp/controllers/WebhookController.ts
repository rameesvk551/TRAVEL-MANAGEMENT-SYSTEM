...
export class WebhookController {
        ...
        try {
            ...
            if (message) {
                ...
                if (tenantId) {
                    ...
                    await this.handleConsentKeywords(tenantId, message.senderPhone, message.textContent?.body);
            ...
            if (status) {
                ...
                const tenantId = await this.resolveTenantId(status.recipientPhone);
                if (tenantId) {
                    await this.audit(tenantId, {
    ...
    sendMessage = async (req: any, res: any, next: any) => {
        try {
            ...
            if (result.success) {
                await this.audit(req.context?.tenantId, {
    ...
    sendMessageV2 = async (req: any, res: any, next: any) => {
        try {
            ...
            if (normalizedType === 'template') {
                ...
                const result = await this.provider.sendTemplate(recipientPhone, templateName, language, components);
                if (!result.success) {
                    res.status(400).json({ success: false, error: result.errorMessage, errorCode: result.errorCode });
                    return;
                }
    ...
    private async handleConsentKeywords(tenantId: string, senderPhone: string, messageBody?: string) {
