...
export class WhatsAppAnalyticsController {
    ...
    getCampaignStats = async (req: any, res: any) => {
        try {
            const { tenantId } = req;
            const { start, end } = req.query;
            const stats = await this.analyticsService.getCampaignStats(
                tenantId,
    ...
    getResponseStats = async (req: any, res: any) => {
        res.json({ avgResponseTimeMinutes: 12, resolutionRate: 92 });
    };
}
