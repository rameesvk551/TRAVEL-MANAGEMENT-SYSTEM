...
export class TimelineService {
    constructor(private timelineRepo: any) { }

    async recordEvent(entityType: string, entityId: string, payload: any) {
        const entry = UnifiedTimelineEntry.create({
            ...
            entryType: payload.entryType || 'SYSTEM_EVENT',
            ...
            metadata: { ...payload, oldValue: payload.oldValue, newValue: payload.newValue },
            occurredAt: new Date(),
        });
        return this.timelineRepo.save(entry);
    }

    async recordWhatsAppMessage(message: any, context: any, direction: string) {
        ...
        const entry = UnifiedTimelineEntry.create({
            ...
            actorId: direction === 'INBOUND'
                ? context.primaryActor.contactId || context.primaryActor.phoneNumber || 'unknown'
    ...
    async recordStatusChange(tenantId: string, entityType: string, entityId: string, previousStatus: string, newStatus: string, changedByUserId: string, changedByName: string, source: string = 'SYSTEM') {
        const entry = UnifiedTimelineEntry.create({
            tenantId,
            leadId: entityType === 'lead' ? entityId : undefined,
            bookingId: entityType === 'booking' ? entityId : undefined,
    ...
    async recordPaymentEvent(tenantId: string, bookingId: string, eventType: string, amount: number, currency: string, actorName: string, actorId: string) {
        const titles: any = {
    ...
    async recordTripEvent(tenantId: string, tripAssignmentId: string, departureId: string, eventType: string, employeeId: string, employeeName: string, description: string, media?: any, location?: any) {
        const titles: any = {
    ...
    async recordMediaUpload(tenantId: string, entityType: string, entityId: string, media: any, uploadedByName: string, uploadedById: string) {
    ...
    async getTimeline(entityType: string, entityId: string, tenantId: string, customerVisible?: boolean) {
        let entries: any[];
        switch (entityType) {
            case 'lead':
    ...
    async getTimelineByConversation(conversationId: string, tenantId: string) {
        return [];
    }
    ...
    getMessageTitle(message: any, direction: string) {
