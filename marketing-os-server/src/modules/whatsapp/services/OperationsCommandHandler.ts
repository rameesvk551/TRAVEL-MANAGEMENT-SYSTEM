...
export class OperationsCommandHandler {
    ...
    registerCommands() {
        return [
            { pattern: /^\/hold\s+(\w+)\s+(\d+)/i, requiredRole: 'SALES_AGENT', description: 'Hold seats: /hold <departure_id> <seats>', handler: async (match: any, ctx: any) => { const [, departureId, seats] = match; try { const result = await this.holdService.createHold({ tenantId: ctx.tenantId, departureId, seatCount: parseInt(seats), source: 'MANUAL', sourcePlatform: 'WHATSAPP', holdType: 'SOFT', createdById: ctx.userId }); return `✅ Hold created!\nDeparture: ${departureId}\nSeats: ${seats}\nExpires: ${result.expiresAt.toLocaleTimeString()}`; } catch (error: any) { return `❌ Failed to create hold: ${error.message}`; } } },
            { pattern: /^\/release\s+(\w+)/i, requiredRole: 'SALES_AGENT', description: 'Release hold: /release <hold_id>', handler: async (match: any, ctx: any) => { const [, holdId] = match; try { await this.holdService.releaseHold(holdId, ctx.tenantId, 'MANUAL_RELEASE'); return `✅ Hold ${holdId} released`; } catch (error: any) { return `❌ Failed to release: ${error.message}`; } } },
            { pattern: /^\/inventory\s+(\w+)/i, requiredRole: 'SALES_AGENT', description: 'Check inventory: /inventory <departure_id>', handler: async (match: any, ctx: any) => { const [, departureId] = match; try { const result = await this.inventoryService.getDepartureWithInventory(departureId, ctx.tenantId); const { departure, inventory } = result; return `📦 Inventory: ${departureId}\nStatus: ${departure.status}\nTotal: ${inventory.total}\nBooked: ${inventory.booked}\nHeld: ${inventory.held}\nAvailable: ${inventory.available}`; } catch (error) { return `❌ Departure not found`; } } },
            { pattern: /^\/close\s+(\w+)/i, requiredRole: 'OPS_MANAGER', description: 'Close departure: /close <departure_id>', handler: async (match: any, ctx: any) => { const [, departureId] = match; try { await this.inventoryService.updateDepartureStatus(departureId, ctx.tenantId, 'CLOSED'); return `✅ Departure ${departureId} closed for bookings`; } catch (error: any) { return `❌ Failed to close: ${error.message}`; } } },
            { pattern: /^\/status\s+(\w+)/i, requiredRole: 'SALES_AGENT', description: 'Booking status: /status <booking_id>', handler: async (match: any, ctx: any) => { const [, bookingId] = match; try { const booking = await this.bookingService.getBooking(bookingId, ctx.tenantId); if (!booking) return `❌ Booking not found`; return `📋 Booking: ${bookingId}\nGuest: ${booking.guestName}\nStatus: ${booking.status}\nDates: ${booking.startDate.toDateString()}\nAmount: ${booking.currency} ${booking.totalAmount}`; } catch (error: any) { return `❌ Error: ${error.message}`; } } },
            ...
            { pattern: /^\/help/i, requiredRole: 'SALES_AGENT', description: 'Show available commands', handler: async (_match: any, ctx: any) => { const available = this.commands.filter((cmd: any) => this.hasPermission(ctx.userRole, cmd.requiredRole)).map((cmd: any) => cmd.description).join('\n'); return `📖 Available Commands:\n\n${available}`; } },
        ];
    }

    async processCommand(message: string, context: any, userRole: string, userId: string) {
        ...
        for (const command of this.commands) {
            ...
            if (match) {
                ...
                await this.messageService.sendText({ tenantId: ctx.tenantId, recipientPhone: ctx.phoneNumber, text: response, senderUserId: 'SYSTEM' });
